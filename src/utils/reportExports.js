  ...
  boqData.forEach((section, sIdx) => {
    ...
    section.items.forEach((item, idx) => {
      const row = worksheet.addRow([
        ...
        item.unit,
        item.qty,
        isUnpriced ? '' : (item.useBenchmark && Number(item.qty) ? (item.total / item.qty) : (item.useBenchmark ? item.benchmark : item.rate)),
...
export const exportToPDF = (projectInfo, boqData, isUnpriced, calculateGrandTotal) => {
  ...
  boqData.forEach((section, sIdx) => {
    ...
    section.items.forEach((item, idx) => {
      const rate = item.useBenchmark && Number(item.qty) ? (item.total / item.qty) : (item.useBenchmark ? item.benchmark : item.rate);
      tableData.push([
