...
const BOQWorkspace = () => {
  const {
    ...
    benchmarkSyncState,
    sectionRowRefs,
    project, isCustomWorkspace, projectStructureType,
    marketRegionLabel, marketRegionDisplay,
    isSelectionStage, hasGeneratedBoq, workspaceVisibleSections,
    selectionCountsBySection, sectionTotalsBySection, calculateGrandTotal,
    workspaceAnalytics, activeProjectSection, activeSectionLineCount,
    activeSectionPricedItems, activeSectionPendingItems, totalSelectedCatalogItems,
    AVATAR_COLORS, benchmarkSyncLabel, activeSheetLabel,
    filteredSections, filteredItemCount, isFilteredView,
    totalItems, workspaceFilterOptions, totalColumnCount,
    sectionHeaderSpan, subtotalLeadingSpan,
    benchmarkRefreshAnalytics, sectionLibraryCounts, activeSectionMeta,
    activeCatalogSection, selectedCatalogItemIdsBySection,
    toggleSection, updateSectionTitle,
    updateItem, handleQuantityChange,
    handleCompletedQuantityChange, handleRateApply, handleCustomPricingSave,
    handleManualRateChange, handleRateSourceChange, openItemDetailPanel,
    openDetailedAnalysis, openCustomPricingStudio, activateCustomPricing,
    handleStructuralImport, handleRegionChange,
    _autoRateProject, refreshBenchmarks,
    refreshSectionBenchmarks, refreshItemBenchmark, toggleVO,
    addItemToSection, duplicateItem, addItemBelow, focusSection,
    handleToggleCatalogSelection, handleSelectVisibleCatalogItems,
    handleClearCatalogSelection, enterSelectionStage, handleGenerateBoq,
    returnToWorkspace, scrollToSection, openFormulaEditor, handleFormulaInputsSave,
    getInitials, getAmountFormula, getRateSourceMeta, getBenchmarkDeltaMeta,
    getBenchmarkEvidenceMeta, getQuantityFeedbackMeta, getAutomationMeta,
    getItemStatusMeta, getQuantityDisplayValue, getQuantitySourceLabel,
  ...
  return (
    <div className="ws-container obsidian-surface">
      <div className="ws-workspace-body staggered-fade-in">
        <div className="ws-workspace-shell glass-panel">
          <BOQBillPanel
            ...
            <div className="ws-compact-header glass-card">
              <div className="ws-compact-header-top staggered-fade-in">
                <div className="ws-compact-header-left">
                  <span className="ws-compact-eyebrow emerald-text-gradient">BOQ-Pro Workbook</span>
      ...
      {/* Table */}
      <div className="ws-table-wrap glass-panel">
        <div
                ...
                <React.Fragment key={section.id}>
                  ...
                  >
                    <td colSpan={sectionHeaderSpan} className="ws-section-cell">
                      <div className="ws-section-inner glass-card">
                        ...
                        <div className="ws-empty-section">
                          ...
                          <div className="ws-empty-section-actions">
                            {sectionMeta.catalogSection && (
                              <button className="ws-btn ws-btn-primary emerald-button" onClick={() => enterSelectionStage(section.id)}>
                      ...
                      <React.Fragment key={item.id}>
                        {showSubcategoryHeader && (
                          <tr className="ws-subcategory-row">
                            <td colSpan={totalColumnCount} className="ws-subcategory-cell glass-card">
      ...
      <style jsx="true">{`
        ...
        .ws-main-pane::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: var(--radius-full); }
        ...
        .ws-compact-stat.glass-card:hover {
          background: #ffffff;
          border-color: #e2e8f0;
        ...
        .ws-compact-stat.glass-card-total {
          background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%);
          border-color: #0f172a;
          box-shadow: 0 12px 24px rgba(15, 23, 42, 0.12);
        }
        .ws-compact-stat.glass-card-total span { color: rgba(255,255,255,0.6); }
        .ws-compact-stat.glass-card-total strong { color: #ffffff; }
        .ws-compact-stat.glass-card-total small { color: rgba(255,255,255,0.5); }
        ...
        .ws-mode-switch { display: flex; background: rgba(255,255,255,0.06); border-radius: var(--radius-sm); overflow: hidden; border: 1px solid rgba(255,255,255,0.1); }
        .ws-mode-btn {
          display: flex; align-items: center; gap: 0.25rem;
          padding: 0.25rem 0.625rem;
          font-size: var(--text-size-xs); font-weight: 700;
        ...
        .ws-stat-label { font-size: var(--text-size-3xs); font-weight: 700; color: rgba(255,255,255,0.35); text-transform: uppercase; letter-spacing: 0.06em; }
        .ws-stat-val { font-size: var(--text-size-base); font-weight: 900; color: white; }
        ...
        .ws-region-sel {
          ...
          color: white;
          font-size: var(--text-size-xs); font-weight: 800;
        ...
        .ws-btn {
          ...
          font-size: var(--text-size-sm); font-weight: 700; cursor: pointer;
        ...
        .ws-table-wrap.glass-panel::-webkit-scrollbar { width: 6px; height: 6px; }
        .ws-table-wrap.glass-panel::-webkit-scrollbar-track { background: #f1f5f9; }
        .ws-table-wrap.glass-panel::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: var(--radius-xs); }
        ...
        .ws-section-letter {
          ...
          font-size: var(--text-size-xs); font-weight: 900;
          border-radius: var(--radius-xs); flex-shrink: 0;
        }
        .ws-section-title-input {
          background: none; border: none; outline: none;
          font-size: var(--text-size-lg); font-weight: 800; color: #0f172a;
          flex: 1; padding: 2px 4px;
          text-transform: uppercase;
          letter-spacing: 0.025em;
        }
        .ws-section-title-input:focus { background: white; border-radius: var(--radius-xs); box-shadow: 0 0 0 2px rgba(59,130,246,0.2); }
        ...
        .ws-section-total {
          font-size: var(--text-size-base); font-weight: 900; color: #1d4ed8;
          margin-left: auto;
        }
        .ws-section-actions {
          display: inline-flex;
        ...
        .ws-vo {
          font-size: var(--text-size-3xs); font-weight: 900;
          background: #fef3c7; color: #92400e;
          padding: 1px 5px; border-radius: var(--radius-xs); flex-shrink: 0;
        }
        ...
        .ws-unit-input { text-align: center; font-weight: 700; text-transform: uppercase; font-size: var(--text-size-sm); color: #64748b; letter-spacing: 0.04em; }
        .ws-qty-input {
          text-align: right;
          font-weight: 700;
          font-size: var(--text-size-lg);
        ...
        .ws-progress-bar {
          ...
          overflow: hidden; font-size: var(--text-size-2xs);
        ...
        .ws-empty-section-title { font-size: var(--text-size-lg); color: #0f172a; display: block; }
        ...
        .ws-subtotal-val {
          ...
          font-weight: 900; font-size: var(--text-size-base);
          color: #1e293b;
        ...
        .ws-add-btn {
          ...
          padding: 0.25rem 0.75rem; border-radius: var(--radius-xs);
          font-size: var(--text-size-sm); font-weight: 600;
          cursor: pointer; transition: all 0.15s;
          width: 100%;
          justify-content: center;
        }
        ...
        .ws-avatar {
          ...
          border-radius: 50%;
          display: flex; align-items: center; justify-content: center;
          font-size: var(--text-size-3xs); font-weight: 800;
        ...
        .collab-title-row h3 { margin: 0; font-size: var(--text-size-2xl); font-weight: 800; }
        .collab-close {
        ...
        .collab-input {
          ...
          font-size: var(--text-size-md); outline: none;
        }
        ...
        .collab-role-select {
          ...
          border-radius: var(--radius-md); font-size: var(--text-size-base); font-weight: 700;
        ...
        .collab-list-label {
          font-size: var(--text-size-2xs); font-weight: 800; color: #94a3b8;
        ...
        .collab-person-avatar {
          ...
          display: flex; align-items: center; justify-content: center;
          font-size: var(--text-size-2xs); font-weight: 800; color: white; flex-shrink: 0;
        }
        .collab-person-info { flex: 1; display: flex; flex-direction: column; }
        .collab-person-email { font-size: var(--text-size-md); font-weight: 600; color: #1e293b; }
        .collab-person-role {
          font-size: var(--text-size-xs); font-weight: 700; color: #94a3b8;
          text-transform: uppercase;
        }
        .collab-remove-btn {
          background: none; border: none; color: #cbd5e1;
          font-size: var(--text-size-3xl); cursor: pointer; padding: 2px 6px; border-radius: var(--radius-xs);
        }
        ...
        .activity-panel-header h4 {
          margin: 0; font-size: var(--text-size-lg); font-weight: 800;
          display: flex; align-items: center; gap: 0.5rem; color: #1e293b;
        }
        .activity-list { flex: 1; overflow-y: auto; padding: 0.75rem; }
        .activity-empty {
          text-align: center; color: #94a3b8;
          font-size: var(--text-size-md); padding: 2rem 0;
        }
        .activity-entry {
          display: flex; gap: 0.625rem; padding: 0.5rem 0;
          border-bottom: 1px solid #f8fafc;
        }
        .activity-icon { font-size: var(--text-size-lg); flex-shrink: 0; margin-top: 2px; }
        .activity-content { display: flex; flex-direction: column; flex: 1; }
        .activity-text { font-size: var(--text-size-base); font-weight: 600; color: #334155; }
        .activity-meta { font-size: var(--text-size-xs); color: #94a3b8; margin-top: 1px; }
        /* Signatures Grid */
        .signatures-grid {
          display: grid;
        ...
        .sig-box {
          ...
          @media (max-width: 768px) {
            ...
            .ws-column-letter small {
              font-size: var(--text-size-3xs) !important;
            }
